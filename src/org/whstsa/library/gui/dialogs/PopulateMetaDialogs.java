...
public class PopulateMetaDialogs {
    ...
    public static void populateMenu(Callback<Integer> callback) {
        Dialog<Map<String, Element>> dialog = new DialogBuilder()
                .setTitle("Auto-Populate Books/Members")
                .addLabel("Please fill in one OR the other")
                .addTextField(BOOKS)
        ...
        DialogUtils.getDialogResults(dialog , (results) -> {
            int amount;
                try {
                    amount = results.get(BOOKS).getString() != null ? Integer.parseInt(results.get(BOOKS).getString()) : Integer.parseInt(results.get(MEMBERS).getString());
                    if (amount > 500) {
                        amount = 500;
                    }
                ...
                displayPopulateTable(amount, results.get(BOOKS).getString() != null);
